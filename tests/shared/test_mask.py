...
def test_mask_context_has_no_capacity_gate():
    jobs, vehicles, state = build_context()

    info = ActionMaskBuilder().build(jobs, vehicles, state)

    assert set(info.keys()) == {"unassigned_job_indices", "vehicles_with_jobs_indices", "vehicle_to_job_indices"}
...
def test_mask_operator_blocks_both_when_no_actions(cpu_config):
    masker          = ActionMasker(cpu_config)
    operator_logits = torch.zeros(4)
    info            = {"unassigned_job_indices": [], "vehicles_with_jobs_indices": [], "vehicle_to_job_indices": {}}
...
def test_mask_vehicle_allows_all_vehicles_for_insert(cpu_config):
    masker         = ActionMasker(cpu_config)
    vehicle_logits = torch.zeros(3)
    info           = {"unassigned_job_indices": [2], "vehicles_with_jobs_indices": [0], "vehicle_to_job_indices": {0: [1]}}
